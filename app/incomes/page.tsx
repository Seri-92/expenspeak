...
export default function Page() {
  ...
  return <main className="container mx-auto max-w-3xl space-y-6 px-4 py-8">
    <header>
      <h1 className="text-3xl font-bold text-slate-900">収入</h1>
      <p className="mt-2 text-sm text-muted-foreground">現在のグループ: {currentGroup.name}</p>
    </header>
    <section className="space-y-5 rounded-2xl border border-sky-200 bg-sky-50/60 p-4 sm:p-6">
      <div>
        <Label htmlFor="income-month">手取りを受け取った月</Label>
        <Input id="income-month" type="month" min="1900-01" max="9998-12" value={month} className="mt-2 max-w-xs"
          onChange={(event) => {
      ...
      <p className="text-sm text-slate-600">一人ずつ、金額が分かったタイミングで保存できます。月を切り替える前に保存してください。</p>
