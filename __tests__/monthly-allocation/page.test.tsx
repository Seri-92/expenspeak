...
test("当月の手取りと前月の生活費を年月付きで区別して表示する", async () => {
  ...
  expect(screen.getAllByText("150,000 円")).toHaveLength(2);
...
test("保存後に取り分を更新し、他の項目の入力途中の値を保持する", async () => {
  ...
  await waitFor(() => expect(screen.getAllByText("150,000 円")).toHaveLength(2));
...
test("読み込み失敗を0円として表示せず再試行できる", async () => {
  ...
  expect(await screen.findAllByText("150,000 円")).toHaveLength(2);
});
